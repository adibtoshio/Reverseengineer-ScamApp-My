...
public class LuaBitmap {
  static WeakHashMap<String, WeakReference<Bitmap>> a = new WeakHashMap<String, WeakReference<Bitmap>>();
  ...
  public static Bitmap getBitmap(LuaContext paramLuaContext, String paramString) {
    ...
    if (paramString.toLowerCase().startsWith("http://") || paramString.toLowerCase().startsWith("https://")) {
      ...
      a.put(paramString, new WeakReference<Bitmap>(bitmap));
      return bitmap;
    } 
    if (paramString.charAt(0) != '/') {
      ...
      bitmap = getLocalBitmap((LuaContext)bitmap, stringBuilder.toString());
    } else {
      bitmap = getLocalBitmap((LuaContext)bitmap, paramString);
    } 
    a.put(paramString, new WeakReference<Bitmap>(bitmap));
    return bitmap;
  }
...
/* Location:              C:\Users\User\Downloads\Telegram Desktop\Jemputan Majlis Perkahwinan-2\c.jar!\com\androlua\LuaBitmap.class
