...
public class LuaLexer {
  ...
  private static final String[] f = new String[] { "Unknown internal scanner error", "Error: could not match input", "Error: pushback value was too large" };
  ...
  private static int b(String paramString, int paramInt, int[] paramArrayOfint) {
    ...
    int j = 0;
    int i = paramInt;
    for (paramInt = j; paramInt < k; paramInt = j + 1) {
      j = paramInt + 1;
      paramArrayOfint[i] = paramString.charAt(paramInt) << 16 | paramString.charAt(j);
      i++;
    } 
    return i;
  }
  ...
  private int f() {
    ...
    stringBuilder.append(']');
    i = str.indexOf(stringBuilder.toString());
    return (i > 0) ? (yylength() - i - this.w - 2) : -1;
  }
  ...
  private boolean g() {
    ...
    int i = this.k.length - this.o;
    int j = this.h.read(this.k, this.o, i);
    if (j == 0)
      throw new IOException("Reader returned 0 characters. See JFlex examples for workaround."); 
    if (j > 0) {
      this.o += j;
      if (j == i && Character.isHighSurrogate(this.k[this.o - 1])) {
        this.o--;
        this.v = 1;
      } 
      return false;
  ...
  static class CharSeqReader extends Reader {
    ...
    CharSeqReader(CharSequence param1CharSequence) {
...
/* Location:              C:\Users\User\Downloads\Telegram Desktop\Jemputan Majlis Perkahwinan-2\c.jar!\com\androlua\LuaLexer.class
