  ...
  void expandReplaceOps(ArrayList<Fragment> paramArrayList) {
    ...
    label31: while (true) {
      if (i < this.mOps.size()) {
        ...
        Fragment fragment = op.fragment;
        int m = fragment.mContainerId;
        int k = paramArrayList.size() - 1;
        j = 0;
        while (true) {
          if (k >= 0) {
            Fragment fragment1 = paramArrayList.get(k);
            if (fragment1.mContainerId == m) {
    ...
    for (j = 0; j < k; j++) {
      ...
      if (m != 0 && m != i) {
        for (i = paramInt1; i < paramInt2; i++) {
          ...
          int n;
          for (n = 0; n < i1; n++) {
            if (((Op)backStackRecord.mOps.get(n)).fragment.mContainerId == m)
              return true; 
          } 
        } 
        i = m;
...
/* Location:              C:\Users\User\Downloads\Telegram Desktop\Jemputan Majlis Perkahwinan-2\c.jar!\android\support\v4\app\BackStackRecord.class
