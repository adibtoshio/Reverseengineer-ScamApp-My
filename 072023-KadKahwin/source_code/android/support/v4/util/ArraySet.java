...
public final class ArraySet<E> implements Collection<E>, Set<E> {
  ...
  private int indexOf(Object paramObject, int paramInt) {
    ...
    if (j >= 0) {
      ...
      if (!paramObject.equals(this.mArray[j])) {
        int m;
        for (m = j + 1; m < k && this.mHashes[m] == paramInt; m++) {
  ...
  private int indexOfNull() {
    ...
    if (j >= 0) {
      ...
      if (this.mArray[j] != null) {
        int m;
        for (m = j + 1; m < k && this.mHashes[m] == 0; m++) {
  ...
  public boolean add(E paramE) {
    ...
    if (j < this.mSize) {
      System.arraycopy(this.mHashes, j, this.mHashes, j + 1, this.mSize - j);
      System.arraycopy(this.mArray, j, this.mArray, j + 1, this.mSize - j);
    } 
    this.mHashes[j] = i;
...
/* Location:              C:\Users\User\Downloads\Telegram Desktop\Jemputan Majlis Perkahwinan-2\c.jar!\android\support\v\\util\ArraySet.class
